...
const calculateMarks = async (req, res) => {
  try {
    ...
    if (!examId) {
      return res.status(400).json({ message: "Exam Id Unavailable" });
    }
    ...
    if (!getStudent) {
      return res.status(400).json({ message: "Something went wrong fetching student Id !" });
    }

    const getExam = await Exams.findById(examId);

    if (!getExam) {
      return res.status(400).json({ message: "Something went wrong fetching Exam Id !" });
    }
